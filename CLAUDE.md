...
│   ├── _auth.tsx               # pathless layout for guarded routes
│   ├── login.tsx
│   └── _auth.dashboard.tsx
...
│   ├── apiClient.ts
│   └── queryClient.ts
├── providers/                  # Composed app-wide providers
│   └── AppProviders.tsx        # Mantine, Auth, QueryClient, Router, etc.
...
- File-name conventions:
  ...
  - Dots in filenames denote nesting (`_auth.dashboard.tsx` → `/dashboard` under the `_auth` layout).
...
- Use the `beforeLoad` hook on a layout route (e.g. `_auth.tsx`) to gate children:
